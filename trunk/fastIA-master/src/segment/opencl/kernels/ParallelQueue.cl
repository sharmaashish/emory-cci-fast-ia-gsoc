/*
    DEBUG <-- can be passed when program is build
*/
//#define DEBUG
...
getWork:
        *gotWork = 0;
        ...
        int queue_index = IN_QUEUE_HEAD(group_id) + thread_id;

        barrier(CLK_LOCAL_MEM_FENCE);

        if(get_local_id(0) == 0){
            IN_QUEUE_HEAD(group_id) += get_local_size(0);
        }

        // Nothing to do by default
        int element = -1;
        if(queue_index < IN_QUEUE_SIZE(group_id)){
            element = queue_data[CURR_IN_QUEUE_OFFSET(group_id) + queue_index];
            *gotWork = 1;
        }
          ...
          __local int* prefix_sum_output)
{
    ...
    for (int d = group_size >> 1; d > 0; d >>= 1)
    {
        ...
        if (tid < d)
        {
            ...
            assert(ai < group_size);
            assert(bi < group_size);
    ...
    if (tid == 0) prefix_sum_output[group_size - 1] = 0;
...
// prefix sum input, prefix_sum_output - size should be equal to group size
                 ...
                 __local int* prefix_sum_output)
{
    ...
    int global_queue_index = OUT_QUEUE_HEAD(group_id);

    // set to the number of values this threard is writing
    prefix_sum_input[thread_id] = elements[0];
    ...
    int queue_index = global_queue_index + prefix_sum_output[thread_id];
    ...
    for(int i = 0; i < elements[0]; i++)
    {
        ...
        if(queue_index + i >= OUT_QUEUE_MAX_SIZE(group_id))
        {
             EXECUTION_CODE(group_id) = 1;
             EXECUTION_CODE_GLOBAL = 1;
        }
        else
        {
            queue_data[CURR_OUT_QUEUE_OFFSET(group_id) + queue_index + i]
                                                            = elements[i + 1];
        }
    }
    ...
    if(thread_id == 0)
    {
        OUT_QUEUE_HEAD(group_id) += prefix_sum_output[group_size-1]
                                  + prefix_sum_input[group_size-1];

        if(OUT_QUEUE_HEAD(group_id) >= OUT_QUEUE_MAX_SIZE(group_id))
        {
            OUT_QUEUE_HEAD(group_id) = OUT_QUEUE_MAX_SIZE(group_id);
            //printf("max exceeded");
        }
    }
    return queue_index;
